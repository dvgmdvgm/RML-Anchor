...
3. **次のプロンプトをそのまま実行してください：** ```/anchor_agent 現在のプロジェクトディレクトリをスキャンして、コンテキストの構築とメモリの正しい設定に役立つデータ（技術データ、ビジネスモデル、デザインルール、およびプロジェクトコンテキストを検索・保存するためのその他の典型的なテンプレート）を探してください``` することで、RLM-Anchorを現在のプロジェクトに安全に導入できます。
